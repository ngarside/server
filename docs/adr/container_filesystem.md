...
**Log files** are record files which are generated by an application at runtime.
Typically it is bad practice for containers to write logs to disk, instead they
should be written to standard output to leave the decision of how to persist
them to the runtime. However there are exceptions to this rule, for example when
one container must read the logs written by another container. They can be
either mutable (when written by the application) or immutable (when read by the
application) and placed in an application-specific directory under `/var/log`,
for example `/var/log/caddy/*`.
...
Note that Git repositories are stored under `/srv` whereas LFS data is stored
