...
FROM docker.io/gitea/gitea:1.25.2 AS gitea
SHELL ["/bin/ash", "-euo", "pipefail", "-c"]
RUN gitea --version | grep -o "[0-9.]*" | { head -n 1; cat >/dev/null; } > /version

FROM golang:1.25.5-alpine as gitea-build
...
FROM docker.io/alpine:3.23.0 AS local
COPY gitea/src/server.sh /usr/bin/entrypoint
RUN chmod +x /usr/bin/entrypoint

FROM docker.io/alpine:3.23.0 AS git-build
